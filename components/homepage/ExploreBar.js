...
const ExploreBar = () => {
    ...
    const data = [
        ...
        {
            heading: "Fungible Tokens (XRP or issued tokens)",
            options: [
                "Wallet",
                "Price Info",
                "Chart",
    ...
    return (
        <GridLayoutProvider>
            <div className="flex flex-row justify-between relative items-center h-[60px] gap-4 mt-[20px] w-full">
                {
                    dropDown && <div className='z-10 border-2 p-4 px-6 border-slate-600 absolute right-0 top-16 bg-black/70 rounded-2xl flex flex-col'>
                        {
                            data.map((data, index) => (
                                <div key={index} className='my-2 flex flex-col'>
                                    <span className='text-slate-500 mb-2'>{data.heading}</span>
                                    <div className='flex flex-col'>
                                        {
                                            data.options.map((option, index) => (
                                                <span key={index} className='bg-white/10 rounded-lg my-1 p-2 hover:bg-white/20 cursor-grab'>
                                                    {option}
                                                </span>
                                            ))
                                        }
                ...
                <div className="toggle-btn flex flex-row justify-center items-center w-[86px] p-[6px] rounded-[16px] bg-[#21212A] px-[6px]">
                ...
                <div onClick={() => showDropDown(!dropDown)} className="flex justify-center items-center w-[45px] h-[45px] rounded-[16px] bg-[#21212A]">
